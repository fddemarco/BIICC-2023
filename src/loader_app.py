...
from dimensions.loader.dataset import Comments, Submissions
...
def app(
    posts_type: str, year: int, working_dir: str, download: bool, clean: bool
) -> None:
    """
    ...
    """
    dataset = (
        Submissions(year, working_dir)
        if posts_type == "submissions"
        else Comments(year, working_dir)
    )
