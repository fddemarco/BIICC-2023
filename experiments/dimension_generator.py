...
class DimensionGenerator:
    ...
    def generate_dimensions_from_seeds_copia(self, seeds):
        return list(map(lambda x: self.generate_dimension_from_seeds_copia([x]), seeds))

    def generate_dimension_from_seeds_copia(self, seeds):
        ...
        aver = self.directions_to_score.index.get_loc(('askteenboys', 'Adulting'))
        if seeds == [("teenagers", "RedditForGrownups")]:
            indxs = [self.directions_to_score.index.get_loc(('askteenboys', 'Adulting')),
                     self.directions_to_score.index.get_loc(('AskTeenGirls', 'blackladies'))]
        if seeds == [("AskMen", "AskWomen")]:
            indxs = [self.directions_to_score.index.get_loc(('askteenboys', 'AskTeenGirls')),
                     self.directions_to_score.index.get_loc(('BeardAdvice', 'badwomensanatomy'))]
        if seeds == [("democrats", "AskTrumpSupporters")]:
            indxs = [self.directions_to_score.index.get_loc(('badwomensanatomy', 'AskThe_Donald')),
                     self.directions_to_score.index.get_loc(('BlackLivesMatter', 'MensRights'))]

        directions = self.directions_to_score.iloc[indxs]
